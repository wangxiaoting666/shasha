...
Memory.set(referName,JSON.encode(data),__time_out||15);},getTimeOut:function(nowDate){var timeout=nowDate+__time_out*24*3600*1000;var date=new Date(timeout);return date;},createGUID:function(){var GUID=(function(){var S4=function(){return(((1+Math.random())*0x10000)|0).toString(16).substring(1);};return(S4()+S4()+"-"+S4()+"-"+S4()+"-"+S4()+"-"+S4()+S4()+S4()).toUpperCase();})();Memory.set('n',GUID,3650);}});ReferObj.init();if(JQ('.tbl_16th_result').length>0){console.log('yes');JQ.ajax({url:'http://cdnec.sasa.com/order-top_sales_members.html',type:'GET',data:{},dataType:'json',timeout:9000,error:function(){console.log('http://cdnec.sasa.com/public/app/site/statics/js_mini/Error loading /order-top_sales_members.html');},success:function(res){if(res){for(var i=0;i<res.length;i++)
...
else{console.log('no');}});miniCart={'show':function(target){target=$(target);if(!target)return;if(this.dialog){this.hide();}
...
window.addEvent('domready',function(){withBrowserStore(function(browserStore){browserStore.get('history',function(v){v=JSON.decode(v);if(!v||!v.length||!JQ('.sidebar').length){miniCart.init();formToCart();}});});setTimeout(function(){console.log('sleep first');fromGetPrice();checkStorage();getMixedName();fromGetMktPrice();},5000);});